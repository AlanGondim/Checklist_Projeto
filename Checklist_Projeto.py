...
METODOLOGIA = {
    ...
    "Finalização": ["Reunião de Encerramento", "TEP", "MV LEARN"]
}
...
# --- LÓGICA IA: STATUS BASEADO EM DATAS ---
def calcular_status_ia(d_ini, d_prod, d_fim):
    try:
        ...
        if hoje < ini: return "🔵 Inicialização"
        if hoje >= fim: return "✅ Finalizado"
        if hoje >= prod: return "🚀 Operação Assistida"
        
        # Lógica de fases intermediárias
        diff_total = (prod - ini).days
        diff_hoje = (hoje - ini).days
        perc_tempo = (diff_hoje / diff_total) if diff_total > 0 else 0
        
        if perc_tempo < 0.3: return "⚙️ Planejamento"
        if perc_tempo < 0.6: return "⚙️ Workshop de Processos"
        return "⚙️ Construção / Go Live"
    except:
        return "⚪ Sem Dados"
...
# CSS: Identidade Azul Marinho e Conectores da Timeline
st.markdown("""
    <style>
    /* Range do Calendário e Multiselect em Azul Marinho */
    div[data-baseweb="calendar"] div[aria-selected="true"],
    div[data-baseweb="calendar"] div[data-highlighted="true"] { background-color: #143264 !important; color: white !important; }
    span[data-baseweb="tag"] { background-color: #143264 !important; color: white !important; }
    
    /* Conector da Timeline Sparkline */
...
if modo == "Checklist Operacional":
    ...
    # Sparkline Timeline com linha azul marinho conectada
    ...
    # --- LÓGICA DE FILTROS COM BOTÃO LIMPAR FUNCIONAL ---
    def reset_filtros():
        st.session_state.d_range_key = [date.today().replace(day=1), date.today()]
        st.session_state.f_selected_key = []

    if 'd_range_key' not in st.session_state: reset_filtros()

    with st.expander("🔍 Filtros de Consulta", expanded=True):
        c1, c2, c3 = st.columns([2, 2, 1])
        data_range = c1.date_input("Período de Auditoria", value=st.session_state.d_range_key, key="d_range_key", format="DD/MM/YYYY")
        fase_filtro = c2.multiselect("Filtrar Fase", list(METODOLOGIA.keys()), default=st.session_state.f_selected_key, key="f_selected_key")
        c3.markdown("<br>", unsafe_allow_html=True)
        c3.button("Limpar Filtros", on_click=reset_filtros, use_container_width=True)
    ...
    if projs:
        ...
        if len(st.session_state.d_range_key) == 2:
            df = df[(df['data_raw'] >= str(st.session_state.d_range_key[0])) & (df['data_raw'] <= str(st.session_state.d_range_key[1])) | (df['data_auditoria'] == "Não Auditado")]
        if st.session_state.f_selected_key:
            df = df[df['Status IA'].apply(lambda x: any(f in x for f in st.session_state.f_selected_key))]

        # --- PLOTAGEM DOS SPARKINES POR FASE (Contabilização) ---
        st.markdown("### 📊 Contabilização de Projetos por Fase")
        spark_cols = st.columns(len(METODOLOGIA))
        for idx, fase in enumerate(METODOLOGIA.keys()):
            count = len(df[df['Status IA'].str.contains(fase)])
            with spark_cols[idx]:
                st.metric(label=fase, value=count)
                # Sparkline visual simulado por barra de progresso azul marinho
                st.progress(min(count/10, 1.0) if count > 0 else 0.0)

        st.markdown("---")
        selecao = st.dataframe(df[['id', 'nome_projeto', 'gerente_projeto', 'Status IA', 'Progresso %', 'data_auditoria']], use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row", column_config={"id": None, "Progresso %": st.column_config.ProgressColumn(format="%.1f%%", color="#143264")})
