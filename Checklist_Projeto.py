...
from datetime import datetime, date # CORREÇÃO: Importação explícita de date
import os

# --- DATABASE SETUP ---
Base = declarative_base()
DB_NAME = 'sqlite:///hub_inteligencia_executivo.db'
# Connect_args para evitar erros de concorrência no SQLite
...
def analisar_status_ia(proj):
    ...
    try:
        ...
        if hoje < d_ini:
            return "Inicialização", ["TAP", "Contrato", "Proposta"]
        elif d_ini <= hoje < d_prod:
            return "Workshop de Processos / Construção", ["Blueprint", "Carga Precursora", "Homologação"]
        elif d_prod <= hoje < d_ter:
            return "Go Live / Operação Assistida", ["Ata Go/No Go", "Treinamento", "Termo de Aceite"]
        else:
            return "Finalização", ["TEP", "Lições Aprendidas"]
    except Exception:
        return "Datas Pendentes", ["Cronograma"]
...
def popup_auditoria(projeto_id):
    ...
    with tab1:
        ...
        for fase, itens in METODOLOGIA.items():
            ...
            with st.expander(f"{fase} - {f_perc:.0f}% Validado", expanded=(f_perc < 100)):
                if st.button(f"✅ Marcar Conformidade Total: {fase}", key=f"aud_all_{fase}"):
                    for item in itens:
    ...
    with tab2:
        ...
        if hist:
            st.table(pd.DataFrame([{"Data": h.data_auditoria, "Auditor": h.responsavel, "Performance": f"{h.progresso_total:.1f}%"} for h in hist]))

    with tab3:
        f_ev = st.selectbox("Fase:", list(METODOLOGIA.keys()))
        up = st.file_uploader("Anexar Evidência")
        if st.button("Salvar Arquivo") and up:
...
if modo == "Checklist Operacional":
    ...
    if st.button("💾 SALVAR NO HUB", use_container_width=True):
        ...
        session.commit(); st.success("Salvo!"); st.balloons()
