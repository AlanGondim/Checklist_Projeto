...
# Garantir colunas novas no banco existente
...
def popup_auditoria(projeto_id):
    ...
    with tab1:
        ...
        if p_medio == 100:
            st.success("🌟 **PROJETO EM CONFORMIDADE INTEGRAL!** Todos os artefatos validados.")
            st.balloons()
        ...
        if st.button("🚀 CONSOLIDAR AUDITORIA", use_container_width=True):
            ...
            session.commit(); st.success("Auditoria Salva!"); st.rerun()
    ...
    with tab3:
        ...
        up = st.file_uploader("Anexar print de e-mail ou PDF", key="up_audit")
...
if modo == "Checklist Operacional":
    ...
    st.markdown("""<style>.timeline-wrapper { position: relative; margin-bottom: 40px; padding-top: 10px; } .timeline-line { position: absolute; top: 38px; left: 5%; right: 5%; height: 3px; background-color: #143264; z-index: 1; } .pie-circle { width: 45px; height: 45px; border-radius: 50%; display: inline-block; position: relative; z-index: 2; background-color: white; border: 3px solid #143264; }</style>""", unsafe_allow_html=True)
    cols_visual = st.columns(len(fases_lista))
    for i, fase in enumerate(fases_lista):
        valor = perc_fases[fase]
        with cols_visual[i]:
            st.markdown(f"<div style='text-align: center;'><div class='pie-circle' style='background: conic-gradient(#143264 {valor}%, #E0E0E0 0);'></div><p style='font-size: 11px; font-weight: bold;'>{fase}</p><p style='font-size: 14px;'>{valor:.0f}%</p></div>", unsafe_allow_html=True)

    if st.button("💾 SALVAR NO HUB", use_container_width=True):
        if nome_p and gp_p:
            novo = Projeto(nome_projeto=nome_p, gerente_projeto=gp_p, regional=reg_p, oportunidade=oportunidade, data_inicio=str(d_inicio), data_termino=str(d_termino), **{MAPA_COLUNAS[f]: v for f, v in perc_fases.items()})
    ...
    if projs:
        ...
        for p in projs:
            ...
            if status_itens:
                progresso_real = (sum(1 for v in status_itens.values() if v) / sum(len(v) for v in METODOLOGIA.values())) * 100
            else:
        ...
        selecao = st.dataframe(df_display[['id', 'nome_projeto', 'gerente_projeto', 'regional', 'Progresso %', 'data_auditoria']], use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row", column_config={"id": None, "Progresso %": st.column_config.ProgressColumn(min_value=0, max_value=100, format="%.1f%%")})
