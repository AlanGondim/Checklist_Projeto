...
def popup_auditoria(projeto_id):
    ...
    with tab1:
        ...
        if st.button("🚀 CONSOLIDAR AUDITORIA", use_container_width=True):
            ...
            for fase in METODOLOGIA.keys():
                count = sum(1 for it in METODOLOGIA[fase] if novos_status.get((f, it)))
    ...
    with tab2:
        ...
        if hist:
            # CORREÇÃO DA DATA: Convertendo para datetime e formatando para DD/MM/YYYY
...
if modo == "Checklist Operacional":
    ...
    st.markdown("""
        ...
        .pie-circle { width: 45px; height: 45px; border-radius: 50%; display: inline-block; position: relative; z-index: 2; background-color: white; }
        </style>
