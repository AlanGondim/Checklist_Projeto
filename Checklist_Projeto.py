...
from datetime import datetime, date # Importação essencial para os gatilhos de data
import os
...
# --- 2. METODOLOGIA E REGRAS DE NEGÓCIO ---
METODOLOGIA = {
...
def analisar_fase_ideal(p):
    ...
    try:
        ...
        elif d_prod <= hoje <= d_fim: return "Go Live/Operação", ["Ata de Go/No Go", "Termo de Aceite"]
...
def popup_auditoria(projeto_id):
    ...
    st.info(f"🤖 **Insight IA:** O projeto deveria estar em **{fase_sugerida}**. Documentos críticos: {', '.join(docs_críticos)}.")
        ...
        for fase, itens in METODOLOGIA.items():
            ...
            with st.expander(f"{fase} - {f_perc:.0f}%", expanded=(f_perc < 100)):
                ...
                for item in itens:
                    ...
                    res = st.checkbox(item, value=v, key=f"aud_chk_{proj.id}_{fase}_{item}")
...
if modo == "Checklist Operacional":
    ...
    with st.container():
        ...
        horas_cont = c2.number_input("Horas", min_value=0.0)
        d_inicio = c3.date_input("Início", format="DD/MM/YYYY")
        d_termino = c1.date_input("Término", format="DD/MM/YYYY")
        d_producao = c2.date_input("Produção", format="DD/MM/YYYY")
        resp_aud = c3.text_input("Responsável Auditoria")
    ...
    for i, fase in enumerate(fases_lista):
        ...
        with cols_v[i]:
            st.markdown(f"<div style='text-align:center'><div class='pie-circle' style='background: conic-gradient(#143264 {v}%, #eee 0); border: 4px solid {cor}'></div><p style='font-size:10px'>{fase}</p></div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    if st.button("💾 SALVAR NO HUB", use_container_width=True):
        novo = Projeto(nome_projeto=nome_p, gerente_projeto=gp_p, regional=regional_p, oportunidade=oportunidade, data_inicio=str(d_inicio), data_termino=str(d_termino), data_entrada_producao=str(d_producao), responsavel_auditoria=resp_aud, **{MAPA_COLUNAS[f]: v for f, v in perc_fases.items()})
        session.add(novo); session.flush()
        for (f, i), v in checks_ops.items():
